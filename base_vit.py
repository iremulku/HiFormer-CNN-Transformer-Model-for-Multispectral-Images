...
def get_b32_config():
    ...
    config.update(dict(dim=768))
    config.update(dict(representation_size=768))
    return config
...
def resize_positional_embedding_(posemb, posemb_new, has_class_token=False):
...
class PositionalEmbedding1D(nn.Module):
    ...
    def forward(self, x):
        ...
        # print("x type",type(x))
        # print("embed type",type(self.pos_embedding.expand(x.shape[0], -1, -1)))
        # print("embed size", self.pos_embedding.expand(x.shape[0], -1, -1).shape)
        
        return x + self.pos_embedding.expand(x.shape[0], -1, -1)
...
class ViT(nn.Module):
    ...
    def __init__(
        ...
        #print("fh and fw", fh, fw)
        gh, gw = h // fh, w // fw  # number of patches
        #print("gh and gw", gh, gw)
        seq_len = gh * gw
    ...
    def forward(self, x):
        ...
        if hasattr(self, 'positional_embedding'):
            # print("sh1", x.shape)
            # print("sh2", self.positional_embedding(x).shape)
